    ...
    private String getMTDTurnosValidos(Date fechaInicial, Date fechaFinal, String estacionId) { //Integer estacionId) {
        ...
        try {
            query = "SELECT d.fecha, t.turno_id, e.nombre "
                    ...
                    + "AND d.estacion_id IN (" + estacionId + ") AND d.fecha BETWEEN TO_DATE('" + sdf_ddmmyyyy.format(fechaInicial) + "', 'dd/mm/yyyy') AND TO_DATE('" + sdf_ddmmyyyy.format(fechaFinal) + "', 'dd/mm/yyyy') "
            ...
            while (rst.next()) {
                if (!uFecha.equals( rst.getString(3).concat(rst.getString(1)) )) {
                    ...
                    uFecha = rst.getString(3).concat(rst.getString(1));
                }
                lFecha = rst.getString(3).concat(rst.getString(1));
...
//            String turnos = getMTDTurnosValidos(fechaInicial, fechaFinal, estacionId);
            query = "SELECT DISTINCT pro.producto_id, pro.nombre, pro.codigo, pro.estado, pro.orden_pos "
                    ...
                    + "AND d.estacion_id = e.estacion_id AND d.estacion_id IN (" + estacionId + ") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') AND t.turno_id IN ("
...
//    public List<String[]> getMTDPrecios(Date fechaInicial, Date fechaFinal, Integer estacionId, String productsIds, Integer tipodespachoId) {
    public List<String[]> getMTDPrecios(Date fechaInicial, Date fechaFinal, String estacionId, String productsIds, Integer tipodespachoId) {
        ...
        try {
            ...
            if (turnos.contains(",") && turnos.split(",").length > 1000) {
                List<String> listT = Arrays.asList(turnos.split(","));
                int sIndex = 0, fIndex = 0, thousandGroups = (listT.size()/1000) +1;
                String turnosCommas;
                for (int i = 0; i<thousandGroups; i++) {
                    sIndex = 1000 * i;
                    fIndex = 1000 * (i+1);
                    turnosCommas = listT.subList(sIndex, fIndex).toString().replace("[", "").replace("]", "");
                    if (i+1==thousandGroups) {
                        turnosCommas = listT.subList(sIndex, listT.size()-1).toString().replace("[", "").replace("]", "");
                        queryComplement += " OR t.turno_id IN ("+ turnosCommas +") ";
                    } else {
                        queryComplement += (i>0) ?  " OR t.turno_id IN ("+ turnosCommas +") " : " t.turno_id IN ("+ turnosCommas +") ";
                    }
                }
                queryComplement = " ("+queryComplement+") ";
            } else {
                queryComplement = " t.turno_id IN (" + turnos + ") ";
            }
            ...
            query = "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, e.nombre, pro.nombre, p.precio, pro.producto_id "
                    ...
                    + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
...
//                    + "AND t.turno_id IN (" + turnos + ") "
                    + "AND " + queryComplement
                    + "AND pro.producto_id in (" + productsIds + ") "
                    + "ORDER BY 2, 1, 5";
            pst = getConnection().prepareStatement(query);
    ...
    public List<String[]> getMTDVolumenes(Date fechaInicial, Date fechaFinal, String estacionId, String productsIds) {
        ...
        try {
            query = "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, pro.producto_id, pro.nombre, SUM(ld.lectura_finaL - ld.lectura_inicial - ld.calibracion), e.nombre "
                    ...
                    + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), pro.producto_id, pro.nombre, e.nombre "
                    //                    + "ORDER BY 1, pro.producto_id"
                    + " UNION "
                    + "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, pro.producto_id, pro.nombre, 0, e.nombre "
                    ...
                    + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') AND pro.producto_id in (" + productsIds + ") "
                    + "GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), pro.producto_id, pro.nombre, e.nombre "
                    + "ORDER BY 5, 1, 2";
            pst = getConnection().prepareStatement(query);
//            pst.setInt(1, estacionId);
...
//            int itemNum = 0;
            ...
            while (rst.next()) {
                if (!fecha.isEmpty() && !fecha.equals( rst.getString(5).concat(rst.getString(1)) )) {
                    tmplist.add(0, fecha);
                    ...
                    fecha = rst.getString(5).concat(rst.getString(1));
    ...
    public List<String[]> getMTDMonetario(Date fechaInicial, Date fechaFinal, String estacionId, String productsIds) {
        ...
        try {
            query = "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, pro.producto_id, pro.nombre, SUM(p.precio * (ld.lectura_final - ld.lectura_inicial - ld.calibracion)), e.nombre "
                    ...
                    + "AND p.turno_id = t.turno_id AND p.producto_id = ld.producto_id AND p.tipodespacho_id = ld.tipodespacho_id AND d.estacion_id = e.estacion_id "
                    + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), pro.producto_id, pro.nombre, e.nombre "
                    //                    + "ORDER BY 1, pro.producto_id"
                    + " UNION "
    ...
    public List<String[]> getMTDVentaOtrosProductos(Date fechaInicial, Date fechaFinal, String estacionId, boolean totalXdia) {
        ...
        try {
            ...
            if (totalXdia && !otrosProductosIds.isEmpty()) {
                query = "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, SUM(ap.monto), e.nombre "
                        + "FROM dia d, turno t, producto p, estacion_producto ep, arqueocaja a, arqueocaja_producto ap, estacion e "
                        + "WHERE d.fecha = a.fecha AND a.arqueocaja_id = ap.arqueocaja_id AND ap.producto_id = p.producto_id AND ep.estacion_id = a.estacion_id AND ep.producto_id = p.producto_id "
                        + "AND d.fecha = t.fecha AND d.estacion_id = t.estacion_id AND t.turno_id = a.turno_id AND d.estacion_id = e.estacion_id "
                        + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') AND p.producto_id IN (" + otrosProductosIds + ") "
                        + "GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), e.nombre "
                        //                    + "ORDER BY 1"
                        + " UNION "
                        + "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, 0, e.nombre "
                        + "FROM dia d, estacion e "
                        + "WHERE d.estacion_id = e.estacion_id "
                        + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
...
//                pst.setInt(4, estacionId);
                ...
                while (rst.next()) {
                    ...
                    result.add(new String[]{
                        rst.getString(3).concat(rst.getString(1)), //fecha
                        rst.getString(2), //ventas
                ...
                query = "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, p.producto_id, SUM(ap.monto), e.nombre "
                        + "FROM dia d, turno t, producto p, estacion_producto ep, arqueocaja a, arqueocaja_producto ap, estacion e "
                        + "WHERE d.fecha = a.fecha AND a.arqueocaja_id = ap.arqueocaja_id AND ap.producto_id = p.producto_id AND ep.estacion_id = a.estacion_id AND ep.producto_id = p.producto_id "
                        + "AND d.fecha = t.fecha AND d.estacion_id = t.estacion_id AND t.turno_id = a.turno_id AND d.estacion_id = e.estacion_id "
                        + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') AND p.producto_id IN (" + otrosProductosIds + ") "
                        + "GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), p.producto_id, e.nombre "
                        + //"--ORDER BY 1, p.producto_id                    " +
                        " UNION "
                        + "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, p.producto_id, 0, e.nombre "
                        + "FROM dia d, producto p, estacion e "
                        + "WHERE d.estacion_id = e.estacion_id AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
...
//                pst.setInt(4, estacionId);
                ...
                String uFecha = "";
                while (rst.next()) {
                    if (!uFecha.isEmpty() && !uFecha.equals( rst.getString(4).concat(rst.getString(1)) )) {
                        datamap.add(0, uFecha);
                        result.add(Arrays.copyOf(datamap.toArray(), datamap.size(), String[].class));
                        datamap = new ArrayList();
                        uFecha = rst.getString(4).concat(rst.getString(1));
    ...
    public Integer getCantidadTurnos(Date fechaInicial, Date fechaFinal, String estacionId) { //Integer estacionId) {
        ...
        try {
            query = "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, COUNT(t.turno_id) "
                    + "FROM dia d, turno t "
                    + "WHERE d.fecha = t.fecha AND d.estacion_id = t.estacion_id AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy') "
                    + "ORDER BY 2 DESC";
            pst = getConnection().prepareStatement(query);
//            pst.setInt(1, estacionId);
    ...
    public List<String[]> getMTDVentaTurnoByFechaEstacion(Date fechaInicial, Date fechaFinal, String estacionId) {
        ...
        try {
            ...
            query = "SELECT virtual.fecha, virtual.turno_id, SUM(virtual.total), virtual.ssnombre "
                    + "FROM ("
                    + "    SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, t.turno_id, SUM(p.precio * (ld.lectura_final - ld.lectura_inicial - ld.calibracion)) total, e.nombre ssnombre "
                    + "    FROM dia d, turno t, lectura l, lectura_detalle ld, precio p, producto pro, estacion e "
                    ...
                    + "    AND ld.producto_id = pro.producto_id AND d.estacion_id = e.estacion_id "
                    + "    AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy')  "
                    + "    GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), t.turno_id, e.nombre  "
                    + "    UNION "
                    + "    SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, t.turno_id, SUM(ap.monto) total, e.nombre "
                    + "    FROM dia d, turno t, producto p, estacion_producto ep, arqueocaja a, arqueocaja_producto ap, estacion e "
                    + "    WHERE d.fecha = a.fecha AND a.arqueocaja_id = ap.arqueocaja_id AND ap.producto_id = p.producto_id AND ep.estacion_id = a.estacion_id AND ep.producto_id = p.producto_id "
                    + "    AND d.fecha = t.fecha AND d.estacion_id = t.estacion_id AND t.turno_id = a.turno_id AND d.estacion_id = e.estacion_id "
                    + "    AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') AND p.producto_id IN (" + otrosProductosIds + ") "
                    + "    GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), t.turno_id, e.nombre "
                    + ") virtual "
                    + "GROUP BY fecha, turno_id, ssnombre "
                    + "ORDER BY 4, 1, 2";
...
//            pst.setInt(4, estacionId);
            ...
            int mysize = 0;
            while (rst.next()) {
                if (!fecha.isEmpty() && !fecha.equals( rst.getString(4).concat(rst.getString(1)) )) {
                    mysize = tmplist.size();
                    ...
                    tmplist.add(0, fecha);
                    result.add(Arrays.copyOf(tmplist.toArray(), tmplist.size(), String[].class));
                    fecha = rst.getString(4).concat(rst.getString(1));
                    tmplist = new ArrayList();
                } else if (fecha.isEmpty()) {
                    fecha = rst.getString(4).concat(rst.getString(1));
                }
    ...
    public List<String[]> getMTDVentaIslaByFechaEstacion(Date fechaInicial, Date fechaFinal, String estacionId) {
        ...
        try {
            ...
            query = "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, b.isla, SUM(ld.lectura_finaL - ld.lectura_inicial - ld.calibracion), e.nombre "
                    ...
                    + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), b.isla, e.nombre  "
                    + " UNION "
                    + "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, b.isla, 0, e.nombre "
    ...
    private Map<String, Double> getTotalxDia(Date fechaInicial, Date fechaFinal, String estacionId) {
        ...
        try {
            ...
            query = "SELECT fecha, SUM(monto), nombre "
                    + "FROM ( "
                    + "    SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, SUM(ad.monto) monto, e.nombre "
                    + "    FROM dia d, arqueocaja a, arqueocaja_detalle ad, estacion e "
                    + "    WHERE d.fecha = a.fecha AND a.arqueocaja_id = ad.arqueocaja_id AND d.estacion_id = a.estacion_id AND d.estacion_id = e.estacion_id "
                    + "    AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy')  "
                    + "    GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), e.nombre "
                    + "        UNION "
                    + "    SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, 0 monto, e.nombre "
                    + "    FROM dia d, estacion e "
                    + "    WHERE NOT EXISTS (SELECT * FROM arqueocaja a, arqueocaja_detalle ad WHERE d.fecha = a.fecha AND a.arqueocaja_id = ad.arqueocaja_id AND d.estacion_id = a.estacion_id ) AND d.estacion_id = e.estacion_id "
                    + "    AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "        UNION "
                    + "    SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, SUM(e.monto) monto, ss.nombre  "
                    + "    FROM dia d, arqueocaja a, efectivo e, estacion ss "
                    + "    WHERE d.fecha = a.fecha AND a.arqueocaja_id = e.arqueocaja_id AND d.estacion_id = a.estacion_id AND d.estacion_id = ss.estacion_id "
                    + "    AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy')  "
                    + "    GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), ss.nombre "
                    + "        UNION "
                    + "    SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, 0 monto, e.nombre "
                    + "    FROM dia d, estacion e "
                    + "    WHERE NOT EXISTS (SELECT * FROM arqueocaja a, efectivo e WHERE d.fecha = a.fecha AND a.arqueocaja_id = e.arqueocaja_id AND d.estacion_id = a.estacion_id ) AND d.estacion_id = e.estacion_id "
                    + "    AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "    ) tmptab "
                    + "GROUP BY fecha, nombre ORDER BY 3, 1";
            pst = getConnection().prepareStatement(query);
//            pst.setInt(1, estacionId);
            pst.setString(1, sdf_ddmmyyyy.format(fechaInicial));
            pst.setString(2, sdf_ddmmyyyy.format(fechaFinal));
...
//                    + "WHERE d.fecha = a.fecha AND a.arqueocaja_id = ad.arqueocaja_id AND ad.mediopago_id = m.mediopago_id AND d.estacion_id = e.estacion_id AND a.estacion_id = e.estacion_id AND mp.pais_id = e.pais_id AND mp.mediopago_id = m.mediopago_id "
                    + "WHERE d.fecha = a.fecha AND a.arqueocaja_id = ad.arqueocaja_id AND ad.mediopago_id = m.mediopago_id AND d.estacion_id = e.estacion_id AND a.estacion_id = e.estacion_id AND m.pais_id = e.pais_id "
                    + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), m.nombre, e.nombre "
                    + " UNION "
                    + "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, m.nombre, 0, e.nombre "
                    + "FROM dia d, mediopago m, estacion e  "
                    + "WHERE d.estacion_id = e.estacion_id "
                    + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
...
//            pst.setInt(4, estacionId);
    ...
    public List<String[]> getMTDEfectivoByFechaEstacion(Date fechaInicial, Date fechaFinal, String estacionId) {
        ...
        try {
            ...
            query = "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, m.nombre, SUM(ad.monto), e.nombre "
                    + "FROM dia d, arqueocaja a, efectivo ad, mediopago m, estacion e " //, mediopago_pais mp "
//                    + "WHERE d.fecha = a.fecha AND a.arqueocaja_id = ad.arqueocaja_id AND ad.mediopago_id = m.mediopago_id AND d.estacion_id = e.estacion_id AND a.estacion_id = e.estacion_id AND mp.pais_id = e.pais_id AND mp.mediopago_id = m.mediopago_id "
                    + "WHERE d.fecha = a.fecha AND a.arqueocaja_id = ad.arqueocaja_id AND ad.mediopago_id = m.mediopago_id AND d.estacion_id = e.estacion_id AND a.estacion_id = e.estacion_id AND m.pais_id = e.pais_id "
                    + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), m.nombre, e.nombre "
    ...
    public List<String[]> getMTDInventarioByFechaEstacion(Date fechaInicial, Date fechaFinal, String estacionId, String invType) {
        ...
        try {
            ...
            if (invType.equals("FINAL")) {
                query = "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, ic.producto_id, ic.final, e.nombre  ";
            } else if (invType.equals("COMPRAS")) {
                query = "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, ic.producto_id, ic.compras, e.nombre  ";
            }
            query = query
                    + "FROM inventario_coco ic, dia d, estacion e "
                    + "WHERE d.fecha = ic.fecha AND d.estacion_id = ic.estacion_id AND d.estacion_id = e.estacion_id "
                    + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + " UNION "
                    + "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, p.producto_id, 0, e.nombre "
                    + "FROM dia d, producto p, estacion e "
                    ...
                    + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') AND p.producto_id IN (" + productosIds + ") "
                    + "GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), p.producto_id, e.nombre "
                    + "ORDER BY 4, 1, 2";
            pst = getConnection().prepareStatement(query);
//            pst.setInt(1, estacionId);
    ...
    public List<String[]> getMTDCalibraciones(Date fechaInicial, Date fechaFinal, String estacionId, String productsIds) {
        ...
        try {
            query = "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, pro.producto_id, SUM(ld.calibracion), e.nombre "
                    ...
                    + "WHERE d.fecha = t.fecha AND d.estacion_id = t.estacion_id  AND d.estacion_id = e.estacion_id AND t.turno_id = l.turno_id AND l.lectura_id = ld.lectura_id AND ld.producto_id = pro.producto_id "
                    + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), pro.producto_id, e.nombre "
                    //                    + "ORDER BY 1, pro.producto_id"
                    + " UNION "
                    + "SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, pro.producto_id, 0, e.nombre "
                    + "FROM dia d, turno t, producto pro, lectura l, estacion e "
                    + "WHERE d.fecha = t.fecha AND d.estacion_id = t.estacion_id  AND d.estacion_id = e.estacion_id AND t.turno_id = l.turno_id "
                    + "AND NOT EXISTS (SELECT * FROM lectura_detalle ld WHERE l.lectura_id = ld.lectura_id AND ld.producto_id = pro.producto_id) "
                    + "AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') AND pro.producto_id in (" + productsIds + ") "
    ...
    public List<String[]> getMTDFaltanteSobrante(Date fechaInicial, Date fechaFinal, String estacionId, String productsIds) {
        ...
        try {
            ...
            query = "SELECT dummy.fecha, SUM(dummy.monto), dummy.nombre  "
                    + "FROM ( "
                    + "    SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, SUM(p.precio * (ld.lectura_final - ld.lectura_inicial - ld.calibracion)) monto, e.nombre  "
                    + "    FROM dia d, turno t, precio p, lectura l, lectura_detalle ld, estacion e  "
                    ...
                    + "    AND ld.lectura_id = l.lectura_id AND ld.estacion_id = d.estacion_id AND ld.producto_id = p.producto_id AND ld.tipodespacho_id = p.tipodespacho_id AND d.estacion_id = e.estacion_id "
                    + "    AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "    GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), e.nombre "
                    + "        UNION "
                    + "    SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, SUM(ap.monto) monto, e.nombre "
                    + "    FROM dia d, turno t, arqueocaja a, arqueocaja_producto ap, estacion e  "
                    ...
                    + "    AND a.arqueocaja_id = ap.arqueocaja_id AND d.estacion_id = e.estacion_id "
                    + "    AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "    GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), e.nombre "
                    + "        UNION "
                    + "    SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, 0 monto, e.nombre  "
                    + "    FROM dia d, estacion e "
                    + "    WHERE NOT EXISTS( SELECT * FROM turno t WHERE d.estacion_id = t.estacion_id AND d.fecha = t.fecha ) AND d.estacion_id = e.estacion_id "
                    + "    AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "    GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), e.nombre "
                    + ") dummy "
                    + "GROUP BY dummy.fecha, dummy.nombre "
                    + "ORDER BY 3, 1";
            pst = getConnection().prepareStatement(query);
//            pst.setInt(1, estacionId);
            pst.setString(1, sdf_ddmmyyyy.format(fechaInicial));
...
//            pst.setInt(7, estacionId);
            ...
            query = "SELECT dtable.fecha, SUM(dtable.monto), dtable.nombre "
                    + "FROM ( "
                    + "    SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, SUM (ad.monto) monto, e.nombre "
                    + "    FROM dia d, turno t, arqueocaja a, arqueocaja_detalle ad, estacion e  "
                    + "    WHERE d.estacion_id = t.estacion_id AND d.fecha = t.fecha AND a.estacion_id = t.estacion_id AND a.turno_id = t.turno_id AND a.fecha = t.fecha "
                    + "    AND a.arqueocaja_id = ad.arqueocaja_id AND d.estacion_id = e.estacion_id "
                    + "    AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "    GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), e.nombre "
                    + "        UNION "
                    + "    SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, SUM(e.monto) monto, ss.nombre "
                    + "    FROM dia d, turno t, arqueocaja a, efectivo e, estacion ss  "
                    + "    WHERE d.estacion_id = t.estacion_id AND d.fecha = t.fecha AND a.estacion_id = t.estacion_id AND a.turno_id = t.turno_id AND a.fecha = t.fecha "
                    + "    AND a.arqueocaja_id = e.arqueocaja_id AND d.estacion_id = ss.estacion_id  "
                    + "    AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "    GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), ss.nombre "
                    + "        UNION "
                    + "    SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, 0 monto, e.nombre "
                    ...
                    + "    GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), e.nombre "
                    + ") dtable "
                    + "GROUP BY dtable.fecha, dtable.nombre "
                    + "ORDER BY 3, 1";
            pst = getConnection().prepareStatement(query);
//            pst.setInt(1, estacionId);
            pst.setString(1, sdf_ddmmyyyy.format(fechaInicial));
...
//            pst.setInt(7, estacionId);
            ...
            while (rst.next()) {
                query = rst.getString(3).concat(rst.getString(1));
                ingresos.put(query, rst.getDouble(2));
            }
    ...
    public List<String[]> getMTDFaltanteSobranteVol(Date fechaInicial, Date fechaFinal, String estacionId, String productsIds) {
        ...
        try {
            query = "SELECT dummy.fecha, dummy.producto_id, SUM(dummy.monto), dummy.nombre "
                    ...
                    + "    GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), pro.producto_id, e.nombre  "
                    + "        UNION "
                    + "    SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, pro.producto_id, 0 monto, e.nombre  "
                    + "    FROM dia d, producto pro, estacion e  "
                    ...
                    //                    + "- <calibracion> "
                    + "         ) monto, e.nombre  "
                    + "    FROM dia d, inventario_coco ic, estacion e "
                    + "    WHERE d.fecha = ic.fecha AND d.estacion_id = ic.estacion_id AND d.estacion_id = e.estacion_id "
                    + "    AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') "
                    + "    GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), ic.producto_id, e.nombre  "
                    + "        UNION "
                    + "    SELECT TO_CHAR(d.fecha, 'dd/mm/yyyy') fecha, p.producto_id, -0 monto, e.nombre  "
                    + "    FROM dia d, producto p, estacion e "
                    + "    WHERE NOT EXISTS ( SELECT * FROM inventario_coco ic WHERE d.fecha = ic.fecha AND d.estacion_id = ic.estacion_id ) AND d.estacion_id = e.estacion_id "
                    + "    AND d.estacion_id IN ("+estacionId+") AND d.fecha BETWEEN TO_DATE(?, 'dd/mm/yyyy') AND TO_DATE(?, 'dd/mm/yyyy') AND p.producto_id IN (" + productsIds + ") "
                    + "    GROUP BY TO_CHAR(d.fecha, 'dd/mm/yyyy'), p.producto_id, e.nombre  "
                    + ") dummy "
                    + "GROUP BY dummy.fecha, dummy.producto_id, dummy.nombre  "
                    + "ORDER BY 4, 1, 2";
            pst = getConnection().prepareStatement(query);
//            pst.setInt(1, estacionId);
            pst.setString(1, sdf_ddmmyyyy.format(fechaInicial));
...
//            Map<Integer, Double> prodVals = new HashMap();
            ...
            while (rst.next()) {
                ...
                tmplist.add(rst.getString(3));
prodSS = rst.getString(2).concat(rst.getString(4));
//                diff = (prodVals.containsKey(rst.getInt(2))) ? prodVals.get(rst.getInt(2)) : 0D;
                diff = (prodVals.containsKey(prodSS)) ? prodVals.get(prodSS) : 0D;
                diff = (diff != 0 && rst.getDouble(3) != 0) ? (rst.getDouble(3) - diff) / diff : 0D;
                prodVals.put(prodSS, rst.getDouble(3));
                tmplist.add(decFmt2.format(diff));
            }
            if (!fecha.isEmpty()) { //si entro en el while
                tmplist.add(0, fecha);

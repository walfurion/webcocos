 ...
 */
public class RptWSM extends Panel implements View {
...
//        currency = user.getPaisLogin().getMonedaSimbolo().concat(". ");
        vlRoot.addComponent(utils.buildHeader("Reporte WSM", true, true));
...
//                : ((user.getPaisLogin() != null) ? user.getPaisLogin() : new Pais()));
    ...
    private void buildButtons() {
        ...
        btnGenerar = new Button("Generar WSM", FontAwesome.FILE_EXCEL_O);
...
System.out.println("*** getExcelStreamResource 1 - "+ new Date());
                    ...
                    String[] midata = new String[]{
                        ...
                        ((Estacion) cbxEstacion.getValue()).getCodigo(),
                        new SimpleDateFormat("MM").format(dfdFechaInicial.getValue()),
                        new SimpleDateFormat("yyyy").format(dfdFechaInicial.getValue()),
                        tmpString
                    };
                    XSSFWorkbook workbook2 = xrg2.generateWSM(null, midata);
...
//loadingw.close();
        ...
        String fileName = "COCOs_WSM_".concat(new SimpleDateFormat("yyyyMMddHHmmss").format(Calendar.getInstance().getTime())).concat(".xlsx");

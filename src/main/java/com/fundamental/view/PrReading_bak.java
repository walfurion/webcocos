 ...
 */
public class PrReading_bak extends VerticalLayout implements View {
    ...
    Button btnGuardar = new Button("Guardar lecturas"),
//            btnAll, btnNone, 
            testButton;
    Label labelStation, labelTurno, labelCreadopor,
            lblUltimoDía = new Label("Último día:"),
            lblUltimoTurno = new Label("Último turno:");
    TextField tfdNameSeller = new TextField("Nombre pistero:"),
            tfdNameChief = new TextField("Jefe de pista:");
    ...
    Container contTurnos = new ListContainer<Turno>(Turno.class, new ArrayList()),
            contPais = new ListContainer<Pais>(Pais.class, new ArrayList()), 
            ctrEmpleado;
    Upload upload;
...
//    List<BombaEmpleadoEstacion> listEmpleados;
...
//        cltToolbar.addComponents(cbxPais, cbxEstacion, dfdFecha, cbxTurno);
//
...
//        root.setExpandRatio(btnGuardar, 0.05f);
...
//        setExpandRatio(root, 1);
    ...
    private void getAllData() {
        ...
        if (user.getPaisLogin() == null && contPais.getItemIds().isEmpty()) {
            contPais = new ListContainer<Pais>(Pais.class, service.getAllPaises());
        }
...
//                    svcTurno.closeConnections();
        ...
        tfdNameSeller.setRequired(true);
        tfdNameSeller.setNullRepresentation("");
        tfdNameSeller.setNullSettingAllowed(false);
        tfdNameSeller.setMaxLength(50);
        tfdNameSeller.addStyleName(ValoTheme.TEXTFIELD_SMALL);

        tfdNameChief.setRequired(true);
        tfdNameChief.setNullRepresentation("");
        tfdNameChief.setNullSettingAllowed(false);
        tfdNameChief.setMaxLength(50);
        tfdNameChief.addStyleName(ValoTheme.TEXTFIELD_SMALL);
...
//                && dia.getEstadoId() == 1 && turno.getEstadoId() == 1) { editar = crearLectura = true; } 
        ...
        btnGuardar.setEnabled(crearLectura);    //habilitado (cerrado)
    }
...
//                }
                ...
                for (Integer id : (List<Integer>) tableManuales.getItemIds()) {
                    ...
                    if (dlectura.getmTotal() < 0) {
                        Notification notif = new Notification("ADVERTENCIA:", "Tiene un registro de ventas negativa ("+dlectura.getBomba()+" -> "+dlectura.getProducto()+")."
...
//                btnGuardar.setEnabled(true);    //habilitado
//            }
//        });
...
//        tablePrecios.setSizeFull();
        tablePrecios.setHeight(191, Unit.PIXELS);
...
//        cltInfo.setSizeUndefined();
        ...
        HorizontalLayout cltToolbar = utils.buildHorizontal("cltToolbar", false, true, true, false);
//        cltToolbar.setSizeUndefined();
        cltToolbar.addComponents(cbxPais, cbxEstacion, dfdFecha, cbxTurno, tfdNameSeller, tfdNameChief);
...
//        hlInputs.setComponentAlignment(labelCreadopor, Alignment.TOP_CENTER);
    ...
    private void defineTableBombas() {
        cbxEmpleado = utils.buildCombobox("Empleado:", "nombre", false, true, ValoTheme.COMBOBOX_SMALL, new ListContainer<Empleado>(Empleado.class, listEmpleados));
...
//                                    dla.seteCalibracion(0D);
                                ...
                                for (DtoLectura dlec : lecturasTurnoActivoElectronicas) {
                                    if (dlec.getBombaId().equals(itemId) && dlec.getProductoId().equals(p.getProductoId())
                                            && dlec.getTipodespachoId().equals(bomba.getTipoDespachoId()) 
                                            //&& dlec.getmFinal() > 0
                                            ) {
                    ...
                    public void valueChange(Property.ValueChangeEvent event) {
                        ...
                        for (Integer i : bombIds) {
                            if (i.equals(itemId)) {
                                lectura = (DtoLectura) ((BeanItem) tableElectronicas.getItem(i)).getBean();
                                lectura.seteVolumen(lectura.geteFinal() - lectura.geteInicial());
                                bcrElectronicas.getItem(itemId).getItemProperty("eVolumen").setValue(lectura.geteFinal() - lectura.geteInicial());
                                //TODO: evaluar la utilidad del siguiente codigo
//                                if (pais.getPaisId() == Constant.CAT_PAIS_GUATEMALA) {
